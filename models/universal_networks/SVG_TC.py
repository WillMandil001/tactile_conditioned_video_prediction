...
class Model:
    ...
    def initialise_model(self):
        ...
        self.frame_predictor = lstm_models.lstm(self.g_dim + 48 + self.z_dim + self.state_action_size, self.g_dim, self.rnn_size, self.predictor_rnn_layers, self.batch_size)
    ...
    def save_model(self):
        torch.save({'encoder': self.encoder, 'decoder': self.decoder, 'frame_predictor': self.frame_predictor,
                    ...
                    self.model_dir + "SVG_TC_model" + self.model_name_save_appendix)
    ...
    def run(self, scene, tactile, actions, gain=0.0, test=False, stage=False):
        ...
        for index, (sample_scene, sample_tactile, sample_action) in enumerate(zip(scene[:-1], tactile[:-1], actions[1:])):
            ...
            if index > self.n_past - 1:  # horizon
                ...
                h_pred = self.frame_predictor(torch.cat([h, z_t, state_action, tactile[index]], 1))  # prediction model
                ...
                h_target = self.encoder(scene[index + 1])[0]   # should this [0] be here????
